...
function nxt_add_dark_mode_early_detection() {
    ?>
    <script>
        (function() {
            function getNxtDarkModeCookie() {
                const result = document.cookie.match('(^|;)\\s*' + 'nxt-dark-mode' + '\\s*=\\s*([^;]+)');
                return result ? result.pop() : null;
            }
            
            const darkModeCookie = getNxtDarkModeCookie();
            const prefersDarkScheme = window.matchMedia('(prefers-color-scheme: dark)').matches;
            const isDarkMode = darkModeCookie !== null ? darkModeCookie === "1" : prefersDarkScheme;
            
            if (isDarkMode) {
                // Apply dark mode immediately to prevent flash
                document.documentElement.classList.add('dark-mode');
                
                // Add a class we can check later to know the initial state
                document.documentElement.dataset.initialDarkMode = 'true';
            }
        })();
    </script>
    <?php
}
add_action('wp_head', 'nxt_add_dark_mode_early_detection', 0);
